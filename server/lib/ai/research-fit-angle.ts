...
function parseLine(raw: string, label: 'FEATURE' | 'FIT' | 'TRIGGER' | 'PROOF'): string | null {
...
export interface DossierEnvelope {
  ...
  // Slice 3 will type this as CachedRoleSlot<OpsDossier>. Placeholder
  // until then so the envelope shape stays stable across slices.
  operations: CachedRoleSlot<unknown> | null
}
...
export function parseCachedDossierEnvelope(
  ...
  return {
    ...
    // Slice 3 will swap this for parseOpsSlot. For now, any data already in
    // the operations slot is ignored at read time — there's no ops dossier
    // shape to validate against yet.
    operations: null,
  }
}
...
// operations → unknown (placeholder until slice 3).
...
export function getDossierSlot(
  envelope: DossierEnvelope,
  role: 'operations',
): CachedRoleSlot<unknown> | null
export function getDossierSlot(
  envelope: DossierEnvelope,
  role: 'engineering' | 'product' | 'gtm' | 'operations' | null,
): CachedRoleSlot<CompanyDossier> | CachedRoleSlot<GtmDossier> | CachedRoleSlot<unknown> | null {
...
export function setDossierSlot(
  envelope: DossierEnvelope,
  role: 'engineering' | 'product' | 'gtm' | 'operations' | null,
  slot: CachedRoleSlot<CompanyDossier> | CachedRoleSlot<GtmDossier>,
): DossierEnvelope {
