...
// completed and written its slot in that window. Re-fetch the envelope at
// write time so we merge into the latest state instead of clobbering
// whatever was just written. This shrinks the cross-role race window from
// the full research duration down to the DB read+write round-trip (~50ms);
// a row-level lock would close it entirely but the current volume doesn't
// warrant the complexity.
//
// Failure to write is non-fatal — we still use the dossier for this draft.
// A re-read failure falls back to priorEnvelope, which is no worse than
// the pre-RMW behavior.
async function researchAndCacheRoleDossier<T>(
  input: PersonalizationInput,
  priorEnvelope: DossierEnvelope,
  config: {
    slotName: string;
    inFlight: Map<string, Promise<T>>;
    runResearch: () => Promise<T>;
    writeSlot: (envelope: DossierEnvelope, dossier: T, researchedAt: Date) => DossierEnvelope;
  },
): Promise<T> {
  const companyId = input.companyId!; // caller checked
  const key = inFlightKey(companyId, input.targetRole);
  const existing = config.inFlight.get(key);
  if (existing) return existing;

  const promise = config.runResearch()
    .then(async dossier => {
      const now = new Date();
      const refetched = await prisma.company
        ...
        .catch(err => {
          console.warn(`Failed to re-read envelope before ${config.slotName} cache write:`, err);
          return null;
        });
      const currentEnvelope = refetched
        ? parseCachedDossierEnvelope(refetched.researchDossier ?? null, refetched.researchedAt ?? null)
        : priorEnvelope;
      const nextEnvelope = config.writeSlot(currentEnvelope, dossier, now);
      await prisma.company
        ...
        .catch(err => {
          console.warn(`Failed to cache ${config.slotName} company dossier:`, err);
        });
      return dossier;
    })
    .finally(() => {
      ...
      config.inFlight.delete(key);
    });

  config.inFlight.set(key, promise);
  return promise;
}
...
async function resolvePersonalization(
  ...
  try {
    ...
    // researchAndCacheRoleDossier needs the full envelope to do its
    ...
    if (input.targetRole === 'gtm') {
      ...
      if (slotIsFresh(envelope.gtm, isEmptyGtmDossier)) {
        ...
        dossier = await researchAndCacheRoleDossier(input, envelope, {
          slotName: 'GTM',
          inFlight: inFlightGtmDossier,
          runResearch: () => {
            ...
            return researchCompanyDossierGtmHybrid({
          ...
          writeSlot: (env, d, researchedAt) =>
            setDossierSlot(env, 'gtm', { dossier: d, researchedAt }),
        });
      }
    ...
    if (input.targetRole === 'operations') {
      ...
      if (slotIsFresh(envelope.operations, isEmptyOpsDossier)) {
        ...
        dossier = await researchAndCacheRoleDossier(input, envelope, {
          slotName: 'ops',
          inFlight: inFlightOpsDossier,
          runResearch: () => researchCompanyDossierOpsHybrid({
            company: input.companyInfo,
            apiKey: input.apiKey,
            exaApiKey: process.env.EXA_API_KEY?.trim() || null,
            tavilyApiKey: process.env.TAVILY_API_KEY?.trim() || null,
          ...
          writeSlot: (env, d, researchedAt) =>
            setDossierSlot(env, 'operations', { dossier: d, researchedAt }),
        });
      }
    ...
    // decision 1. Hardcoded 'engineering' as the slot for product too —
    // slotForRole inside setDossierSlot maps both to the eng slot.
    let dossier: CompanyDossier;
    if (slotIsFresh(envelope.engineering, isEmptyDossier)) {
      dossier = envelope.engineering!.dossier;
    } else {
      dossier = await researchAndCacheRoleDossier(input, envelope, {
        slotName: 'engineering',
        inFlight: inFlightEngDossier,
        runResearch: () => {
          ...
          const recencyDays = Number.isFinite(envRecency) && envRecency > 0 ? envRecency : undefined;
          return researchCompanyDossierHybrid({
            company: input.companyInfo,
            apiKey: input.apiKey,
            exaApiKey: process.env.EXA_API_KEY?.trim() || null,
            tavilyApiKey: process.env.TAVILY_API_KEY?.trim() || null,
        ...
        writeSlot: (env, d, researchedAt) =>
          setDossierSlot(env, 'engineering', { dossier: d, researchedAt }),
      });
    }
