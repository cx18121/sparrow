...
**Five standalone ingestion scripts (YC JSON API, Apollo enrichment, Product Hunt GraphQL, Apollo client wrapper, poll orchestrator) wiring all data sources into the shared Company/Contact pool via idempotent upserts.**
...
- **Tasks:** 3 (1a YC, 1b Apollo+PH, 2 Poll)
- **Files modified:** 7 (5 new scripts + package.json + package-lock.json)
...
3. **Task 2: Polling orchestrator** - `8b92a6e` (feat)
