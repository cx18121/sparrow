...
- **`stage=null` reduction (user-requested 2026-05-11) — DONE 2026-05-12c.** Combined Phase A+B dropped null-stage verified rows **from 4,367 → 2,543 (44.1% → 25.7%, -18.4pts / -1,824 rows).** Phase A (tag-based inference, 2026-05-12b) accounted for -17.5pts; Phase B (source-of-truth re-scrape, 2026-05-12c) for -0.9pts plus replacing some Phase-A-inferred Series C+ stages with granular source-of-truth Series C/A/B from Costanoa.
  - Phase B adapter outcomes: **Spark** — no-op, STATE.md was wrong; probe confirmed zero stage strings on the page, existing adapter "no stage data" comment is accurate. **Felicis** — stage decoded from Next.js streaming chunks via chunk-id → title map (4 stages: Seed/Series A/B/C); 29 of 40 featured-company refs captured (remaining 11 fall outside the 1500-char back-search window — diminishing returns to widen). **Costanoa** — stage extracted from Prismic `data.text_cells[]` preferring the cell captioned "COSTANOA'S Initial investment"; 88 of 91 ingested rows carry stage (96.7%).
