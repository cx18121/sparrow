...
import { CANONICAL_STAGES, type CanonicalStage } from "./stages.js";
import { callClaude } from "../../server/lib/ai/anthropic.js";

// Shared pipeline for the stage + location backfill scripts. Both
// backfill-stage-location-tavily.ts and -exa.ts call runBackfill() with
// their own BackfillBackend; the shell here owns the regex pass, the
// Haiku JSON fallback, the concurrency runner, the DB update, and the
// stats printout. The backend covers only the deltas that are genuinely
// per-backend: search invocation, tag names, env-key, optional CLI flags.
...
// search() returns a discriminated union so the runner doesn't need to know
// about backend-specific exception types. 'abort' is the hard-stop case
// (Tavily quota / auth failure) — peer workers stop pulling and the row
// stays untagged so a future resume retries it. 'error' is per-row transient
// failure: tag tried so we don't re-pay on the next run.
...
export interface BackfillBackend {
  name: string;
  // Tags written to Company.tags. tagTried gates one-shot re-runs (NOT IN
  // filter); tagEnriched marks rows that actually got data filled.
  tagTried: string;
  tagEnriched: string;
  // Used for env-var presence check + the error message.
  apiKeyEnv: string;
  // Default result count for the search call (Tavily defaults 4, Exa 5).
  defaultResults: number;
  buildQuery: (c: CompanyRow) => string;
  search: (args: { query: string; apiKey: string; numResults: number }) => Promise<SearchOutcome>;
  // One-line summary printed before the run starts. Backends use this to
  // surface backend-specific params (Exa's recency window, etc.).
  paramsSummary: (defaults: { concurrency: number; results: number; model: string }) => string;
  // Optional: backend-specific CLI flag handling.
  //   extraNotTags adds to the WHERE NOT { tags: { has: <tag> } } chain
  //     (Exa uses this for --skip-tavily-tried).
  //   sourceFilter restricts the WHERE clause to a single Company.source
  //     (Exa-only --source flag).
  parseExtraFlags?: (argv: string[]) => { extraNotTags?: string[]; sourceFilter?: string | null };
}

// ── Regex extraction ──────────────────────────────────────────────────────

// Stage regex: anchor on the canonical phrasing actually used in press
// releases. "raised a Series B", "closed its Series C round", "Seed round
// of $X", "Pre-Seed funding". Plain "Series A" with no surrounding context
// is included too — it's almost always about the round in this domain.
//
// The matchAll iterator inside extractStageRegex builds the Series regex
// fresh each call. Don't hoist it module-level with the `g` flag — JS
// regex state (lastIndex) is shared across concurrent workers and corrupts
// the iterator.
...
//             Aviv-Yafo", "New York")
//   group 2 — country/state (optional): up to 3 title-cased tokens after
//             a comma. Strictly title-cased so a trailing lowercase verb
...
// company called "Series Group" doesn't trigger STAGE_RE_SERIES, and a
// company called "Based (San Francisco) Inc." doesn't fake a location hit.
...
function extractStageRegex(blob: string): CanonicalStage | null {
  ...
  for (const m of blob.matchAll(/\bSeries\s+([A-F])\b/gi)) {
    candidates.push(`Series ${m[1].toUpperCase()}`);
  }
...
interface Stats {
  ...
  fullWhiff: number;       // search returned content but neither field extracted
  writes: number;
}
...
async function processOne(
  ...
  const outcome = await backend.search({
    query: backend.buildQuery(row),
    apiKey: searchApiKey,
    numResults,
  });

  // Abort: rethrow so the runner aborts the whole pass. Row stays untagged
  // so a future resume retries it.
  if (outcome.kind === "abort") {
    throw new Error(outcome.message);
  }

  // Search error: tag tried (don't keep paying on every re-run) and bail.
  if (outcome.kind === "error") {
    stats.searchErrors++;
    console.warn(`  [${row.name}] ${backend.name} error: ${outcome.message}`);
    if (!dryRun) {
      await prisma.company.update({
        where: { id: row.id },
        data: { tags: { push: backend.tagTried } },
      });
    }
    return;
  }
  ...
  const blob = maskName(outcome.blob, row.name);
  ...
  const tagsToAdd = [backend.tagTried];
  if (stage || location) tagsToAdd.push(backend.tagEnriched);
...
// (set when a worker throws — e.g. backend.search returned 'abort').
...
export async function runBackfill(backend: BackfillBackend): Promise<void> {
  const apiKey = process.env.ANTHROPIC_API_KEY?.trim();
  const searchApiKey = process.env[backend.apiKeyEnv]?.trim();
  if (!apiKey) throw new Error("ANTHROPIC_API_KEY is required");
  if (!searchApiKey) throw new Error(`${backend.apiKeyEnv} is required`);
  ...
  const extras = backend.parseExtraFlags?.(process.argv) ?? {};
  ...
  // NOT clauses: always exclude this backend's own tried tag (one-shot
  // guard). Backends can add more via parseExtraFlags (e.g. Exa's
  // --skip-tavily-tried).
  const notFilter: Array<Record<string, unknown>> = [
    { tags: { has: backend.tagTried } },
    ...(extras.extraNotTags ?? []).map(t => ({ tags: { has: t } })),
  ];

  const all = await prisma.company.findMany({
    where: {
      isVerified: true,
      ...(extras.sourceFilter ? { source: extras.sourceFilter } : {}),
      ...nullPredicate,
      NOT: notFilter,
    },
    select: {
  ...
  // --max-calls is the credit-safety cap. Applied AFTER --limit if both
  // are present (--limit defines the target window; --max-calls bounds
  // per-invocation spend within it).
  ...
  if (maxCalls !== null) console.log(`${backend.name} call cap: ${maxCalls}`);
  console.log(backend.paramsSummary({ concurrency, results: backend.defaultResults, model: MODEL }));

  const stats: Stats = {
    scanned: 0,
    searchResultsEmpty: 0, searchErrors: 0,
  ...
  const completed = await runConcurrent(
    ...
    async row => {
      stats.scanned++;
      await processOne(row, backend, apiKey, searchApiKey, backend.defaultResults, stats, dryRun);
    },
    abort,
  );
  ...
  if (!completed) {
    ...
    console.error(`   No ${backend.tagTried} tags applied to in-flight rows; rerun after resolving.`);
  }
  ...
  console.log(`${backend.name} empty results:       ${stats.searchResultsEmpty}`);
  console.log(`${backend.name} errors:              ${stats.searchErrors}`);
